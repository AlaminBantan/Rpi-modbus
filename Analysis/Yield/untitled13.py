# -*- coding: utf-8 -*-
"""
Created on Mon Jan  8 14:49:38 2024
...
# Group data by 'Treatment' and 'Date' and calculate the mean of 'fruits/stem' for each group
grouped_data = df.groupby(['Treatment', 'Date'])['fruits/stem'].mean().reset_index()
...
plt.title('Fruits per Stem Over Time in Zone B')
plt.xlabel('Date')
plt.ylabel('Fruits per Stem')
plt.xticks(bar_positions + bar_width * (len(treatments_order) - 1) / 2, df['Date'].unique(), rotation=45, ha='right')
plt.legend()

# Save the figure as an SVG file
save_path = r"C:\Users\bantanam\KAUST\CDA-CEA Team - Documents\CO2 misting - Cucumber trial\Results\Zone B\fruits_per_stem_bar_chart.svg"
