...
# Group data by 'Treatment' and 'Date' and sum the '# fruits' for each group
grouped_data = df.groupby(['Treatment', 'Date'])['# fruits'].sum().reset_index()
...
plt.title('# Fruits Over Time for Each Treatment in zone B')
plt.xlabel('Date')
plt.ylabel('# Fruits')
...
save_path = r"C:\Users\bantanam\KAUST\CDA-CEA Team - Documents\CO2 misting - Cucumber trial\Results\Zone B\fruits_bar_chart.svg"
